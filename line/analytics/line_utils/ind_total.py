...
class IndividualTotalCatcher(Catcher):
    async def search_individual_1_total_rate(self, statistic_name: str, league_name: str):
        for coeff_set in self.coefficients[statistic_name]['total_1_&coefficient']:
            ...
            tu_similar_current_result_low = self.total_calculation(
                coeff_set=coeff_set,
                seq=self.home_structure.
                similar_command_individual_total_current_home_command_in_home_away_games_low)
            tu_similar_opposing_result_low = self.total_calculation(
                coeff_set=coeff_set,
                seq=self.away_structure.
                similar_command_individual_total_opposing_teams_current_away_in_home_away_games_low)
            tu_similar_current_result_high = self.total_calculation(
                coeff_set=coeff_set,
                seq=self.home_structure.
                similar_command_individual_total_current_home_command_in_home_away_games_high)
            tu_similar_opposing_result_high = self.total_calculation(
                coeff_set=coeff_set,
                seq=self.away_structure.
                similar_command_individual_total_opposing_teams_current_away_in_home_away_games_high)
            to_similar_current_result_low = self.total_calculation(
                coeff_set=coeff_set,
                seq=self.home_structure.
                similar_command_individual_total_current_home_command_in_home_away_games_low)
            to_similar_opposing_result_low = self.total_calculation(
                coeff_set=coeff_set,
                seq=self.away_structure.
                similar_command_individual_total_opposing_teams_current_away_in_home_away_games_low)
            to_similar_current_result_high = self.total_calculation(
                coeff_set=coeff_set,
                seq=self.home_structure.
                similar_command_individual_total_current_home_command_in_home_away_games_high)
            to_similar_opposing_result_high = self.total_calculation(
                coeff_set=coeff_set,
                seq=self.away_structure.
                similar_command_individual_total_opposing_teams_current_away_in_home_away_games_high)
            ...
            if big_data_current_result and big_data_opposing_result:
                ...
                if big_data_current_under_percent and big_data_opposing_under_percent:
                ...
                if big_data_current_over_percent and big_data_opposing_over_percent:
                    await self.search_kush_rate(statistic_name=statistic_name,
                                                league_name=league_name,
                                                coefficients=self.coefficients,
                                                coeff_set=coeff_set,
